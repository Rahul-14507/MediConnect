...
import { Loader2, Building2, Plus, ArrowUpRight } from "lucide-react";
    ...
    const createOrgMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            toast({
                ...
                description: `Created ${data.organization.name}. Admin: ${data.admin.getEmployeeId} / password`,
            });
            setOpen(false);
            form.reset();
        },
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage registered organizations and onboard new partners.</p>
                </div>
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Add a new Hospital, Pharmacy, or Lab. An admin account will be auto-generated.
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit((data) => createOrgMutation.mutate(data))} className="space-y-4">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input placeholder="e.g. Metro Diagnostics" {...field} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="METRO" {...field} onChange={e => field.onChange(e.target.value.toUpperCase())} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                ...
                                <DialogFooter className="pt-4">
                                    <Button type="submit" disabled={createOrgMutation.isPending} className="w-full gradient-primary">
                ...
                <CardContent>
                    <Table>
                        ...
                        <TableBody>
                            {orgs?.map((org) => (
                                <TableRow key={org.id}>
                                    <TableCell className="font-mono font-medium">{org.code}</TableCell>
                                    <TableCell className="font-medium">{org.name}</TableCell>
                                    <TableCell>
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize
                                            ${org.type === 'hospital' ? 'bg-blue-100 text-blue-800' :
                                                org.type === 'pharmacy' ? 'bg-emerald-100 text-emerald-800' :
                                                    org.type === 'lab' ? 'bg-purple-100 text-purple-800' :
                                                        'bg-slate-100 text-slate-800'}`}>
                                            {org.type}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-muted-foreground">{org.address || "N/A"}</TableCell>
